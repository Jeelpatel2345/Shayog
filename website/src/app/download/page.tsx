...
export default function WebsiteDownloadPage() {
  const [copied, setCopied] = useState(false);
  const [downloading, setDownloading] = useState(false);

  const appUrl = 'https://shayog-rb55.vercel.app/download';
  const shareText = 'Download the SahYog App (Android APK) for reliable home & community services: https://shayog-rb55.vercel.app/download';
  ...
  return (
    <div className="min-h-[85vh] bg-slate-50 flex flex-col items-center justify-center p-4 sm:p-6 text-slate-900">
      <div className="w-full max-w-md bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-xl flex flex-col items-center text-center">
        ...
        <p className="text-xs text-slate-500 mt-2 max-w-xs leading-relaxed">
          Install the official SahYog native app directly onto your phone. Opens without browser bars, with real-time audio chimes, gate OTP, and GPS worker tracking!
        </p>
        ...
        <div className="w-full my-5 p-3.5 bg-slate-50 rounded-2xl border border-slate-200/80 text-left space-y-2">
          ...
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-800">
            <CheckCircle2 className="w-4 h-4 text-teal-600 flex-shrink-0" />
            <span>Instant sound alerts & push notifications for workers</span>
          </div>
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-800">
            <CheckCircle2 className="w-4 h-4 text-teal-600 flex-shrink-0" />
        ...
        <div className="mt-4 pt-3 border-t border-slate-100 w-full text-center">
          <Link href="/" className="text-[11px] hover:text-teal-700 font-medium text-slate-400">
            Back to Home Page →
          </Link>
        </div>
      </div>
